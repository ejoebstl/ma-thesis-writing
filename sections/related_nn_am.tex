...
\]
This expression can be differentiated with respect to the posterior probability $p^*(s_{U,t}|o_{U,t})$ to calculate a gradient for training a neural network model with backpropagation. Again, let $p^*_{s_{j,t}}$ be $p^*(s_{j,t}|o_j)$.
\begin{align*}
...
\begin{align*}
\frac{\partial\mathcal{L}_{\text{MMI}}}{\partial p^*_{s_{j,t}}} &= p(x_t = s_{j,t}|O) - p^*_{s_{j,t}}
\end{align*}
...
\begin{align}
\label{eq:mmi_grad}
\frac{\partial\mathcal{L}_{\text{MMI}}}{\partial p^*_{s_{j,t}}} &= \gamma_t(j) - p^*_{s_{j,t}}
\end{align} 
In literature, particularly \cite{ghoshal2013sequence}, a variant is sometimes used: 
\begin{align}
\label{eq:mmi_grad_simple}
\frac{\partial\mathcal{L}_{\text{MMI}}}{\partial p^*_{s_{j,t}}} &= \gamma_t(j) - \delta_{ij}
\end{align} 
Here $i$ denotes the index of the correct state at time $t$, given by a viterbi pass. This variant is motivated by the assumption that all $p^*_{s_{U,t}}$ for the correct sequence $S_U$ are equal to one. 
